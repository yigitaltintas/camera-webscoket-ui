...
function App() {
    ...
    async (
      ...
      try {
        ...
        if (response.success) {
          addLog(
            "success",
            `Connecting to ${options.alias || camera.model}...`
          );
        } else {
  ...
  const handleStartContinuous = useCallback(
    async (cameraId: number, intervalSeconds: number) => {
      try {
        const response = await sendMessage({
        ...
        if (response.success) {
          addLog(
            "success",
            `Continuous mode started (every ${intervalSeconds}s)`,
            cameraId
          );
          ...
          addLog(
            "error",
            response.error || "Failed to start continuous mode",
            cameraId
          );
        }
      } catch (error) {
    ...
    async (cameraId: number) => {
      try {
        ...
        if (response.success) {
          ...
          addLog(
            "error",
            response.error || "Failed to stop continuous mode",
            cameraId
          );
        }
      } catch (error) {
    ...
    async (cameraId: number) => {
      try {
        ...
        if (response.success) {
          ...
          addLog(
            "error",
            response.error || "Failed to disconnect camera",
            cameraId
          );
        }
      } catch (error) {
  ...
  const handleSetTransferPath = useCallback(
    async (path: string) => {
      try {
        const response = await sendMessage({
  ...
  const handleGetCameraSettings = useCallback(
    async (cameraId: number) => {
      try {
        const response = await sendMessage({
  ...
  const handleSetCameraSetting = useCallback(
    async (
      settingType: string,
      settingValue: string,
      options: {
      ...
      try {
        ...
        if (response.success) {
          const target = options.applyToAll
            ? "all cameras"
            : options.cameraId
            ? `camera ${options.cameraId}`
            : "master camera";
          addLog(
            "success",
            `${settingType} set to ${settingValue} on ${target}`
          );
        } else {
  ...
  const handleSyncMasterSettings = useCallback(
    async (settingsToSync?: string) => {
      try {
        const command: any = {
        ...
        if (response.success) {
          addLog(
            "success",
            `Synchronized ${response.synced_settings.join(", ")} to ${
              response.total_synced
            } cameras`
          );
        } else {
  ...
  const handleEnableAutoSync = useCallback(
    async (enabled: boolean, intervalMs?: number, settingsToSync?: string) => {
      try {
        const command: any = {
  ...
  const handleGetAvailableSettings = useCallback(
    async (cameraId: number, settingType: string) => {
      try {
        const response = await sendMessage({
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className="text-sm text-slate-300">
                {
                  connectedCameras.filter((c) => c.status === "connected")
                    .length
                }{" "}
                Active
              </span>
